...
const ChartSection: React.FC<{ className?: string }> = ({
  ...
  useEffect(() => {
    (async () => {
      try {
        const [actualRes, predRes, todayRes] = await Promise.all([
          axios.get<SeriesPoint[]>(`/api/dataformodel/${viewMode}`),
          axios.get<SeriesPoint[]>(`/api/predicted/${viewMode}`),
          axios.get<{ totalPlates: number; totalEarning: number }[]>(
            "/api/servings"
          ),
        ]);
    ...
    return (
      <div className="rounded-md bg-white/90 p-3 shadow-md">
        ...
        {[
          ...
          <p
            key={k}
            className="mb-1 flex items-center text-xs text-gray-600"
          >
